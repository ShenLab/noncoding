...
def get_base_model(max_len):
  model_in = Input(shape=(max_len,4))

  conv1 = Conv1D(100, kernel_size=8, activation='relu', input_shape=(max_len,4), kernel_regularizer=l2(.01))(model_in)
  ...
  model_out = Dense(1, activation="sigmoid")(dense3)
  model = Model(inputs=model_in, outputs=model_out)

  return model

# model with gene expression input
def get_gene_model(max_len):
  model_in = Input(shape=(max_len,4))
...
def get_annot_model(max_len):
  ...
  model_annot_out = Dense(1, activation="sigmoid")(mult_layer_annot)
...
def get_gene_annot_model(max_len):
  model_in = Input(shape=(max_len,4))
  rank_in = Input(shape=(1,))
  annot_in = Input(shape=(6,))
